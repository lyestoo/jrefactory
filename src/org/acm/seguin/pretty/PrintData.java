...
public class PrintData {
	...
	private boolean cBlockStyle = true;
	private boolean methodCBlockStyle = true;
	private boolean classCBlockStyle = true;

	// Set to true after method parameters are formatted, before the
	// opening brace
	private boolean isMethodOpenBrace = false;
	...
	private boolean isClassOpenBrace = false;
	...
	public PrintData(OutputStream out)
	{
		...
		try {
			cBlockStyle = bundle.getString("block.style").equalsIgnoreCase("C");
		}
		catch (MissingSettingsException mre) {
			//  Default is sufficient
		}
		...
		try {
			methodCBlockStyle = bundle.getString("method.block.style").equalsIgnoreCase("C");
		}
		catch (MissingSettingsException mre) {
			methodCBlockStyle = cBlockStyle;
		}

		try {
			classCBlockStyle = bundle.getString("class.block.style").equalsIgnoreCase("C");
		}
		catch (MissingSettingsException mre) {
			classCBlockStyle = methodCBlockStyle;
		}
	 ...
	 */
	public void setClassCStyleBlock(boolean value)
	{
		classCBlockStyle = value;
	}
	...
	{
		try {
			return bundle.getBoolean("else.start.line");
		}
		catch (MissingSettingsException mse) {
			return true;
		}
	 ...
	 */
	public void classOpenBrace()
	{
		// This is reset to false in beginBlock after formatting
		isClassOpenBrace = true;
	}
	...
	{
		boolean currentStyle = getCurrentBlockStyle();
		...
		isMethodOpenBrace = false;
		isClassOpenBrace = false;
		newline();
		incrIndent();
	}
	 ...
	 */
	private boolean getCurrentBlockStyle()
	{
		if (isClassOpenBrace) {
			return classCBlockStyle;
		}

		if (isMethodOpenBrace) {
			return methodCBlockStyle;
		}
