...
public class PrettyPrintVisitor implements JavaParserVisitor {
	...
	public Object visit(ASTFieldDeclaration node, Object data)
	{
		...
		if (printData.isDynamicFieldSpacing(node.isJavadocPrinted())) {
			int current = 0;
			if (next.jjtGetChild(0) instanceof ASTPrimitiveType) {
				current = ((ASTPrimitiveType) next.jjtGetChild(0)).getName().length();
			}
			else {
				current = ((ASTName) next.jjtGetChild(0)).getName().length();
			}
	...
	public Object visit(ASTIfStatement node, Object data)
	{
		...
		if (hasElse) {
			...
			node.jjtAccept(specialTokenVisitor, new SpecialTokenData(node.getSpecial("else"), printData, printData.isElseOnNewLine()));
